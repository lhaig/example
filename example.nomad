...
job "example" {
  ...
  migrate {
    ...
    max_parallel = [[.migrate.max_parallel]]
    ...
    min_healthy_time = "[[.migrate.min_healthy_time]]"
    ...
    healthy_deadline = "[[.migrate.healthy_deadline]]"
  }
  ...
  group "cache" {
    ...
    restart {
      ...
      attempts = [[.group.restart.attempts]]
      interval = "[[.group.restart.interval]]"

      # The "delay" parameter specifies the duration to wait before restarting
      # a task after it has failed.
      delay = "[[.group.restart.delay]]"

      # The "mode" parameter controls what happens when a task has restarted
      # "attempts" times within the interval. "delay" mode delays the next
      # restart until the next interval. "fail" mode does not restart the task
      # if "attempts" has been hit within the interval.
      mode = "[[.group.restart.mode]]"
    }
    ...
    ephemeral_disk {
      ...
      sticky = [[.group.ephemeral_disk.sticky]]
      #
      # Setting migrate to true results in the allocation directory of a
      # sticky allocation directory to be migrated.
      migrate = [[.group.ephemeral_disk.migrate]]
      #
      # The "size" parameter specifies the size in MB of shared ephemeral disk
      # between tasks in the group.
      size = [[.group.ephemeral_disk.size]]
    }
    ...
    task "redis" {
      ...
      config {
        image = [[.group.task.config.image]]

        port_map {
          db = [[.group.task.config.port_map]]
        }
      }
      ...
      #
      logs {
        max_files     = [[.group.task.logs.max_files]]
        max_file_size = [[.group.task.logs.max_file_size]]
      }
      ...
      #
      resources {
        cpu    = [[.group.task.resources.cpu]]
        memory = [[.group.task.resources.memory]]

        network {
          mbits = [[.group.task.resources.network.mbits]]
          port "db" {}
        }
      }
      ...
      #
      service {
        name = "[[.group.task.service.name]]"
        tags = [[.group.task.service.tags]]
        port = "db"
